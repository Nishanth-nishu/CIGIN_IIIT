...
def train_and_evaluate_model(model, model_name, config, train_loader, valid_loader, test_loader):
    ...
    scheduler = ReduceLROnPlateau(optimizer, patience=config['scheduler_patience'], mode='min', verbose=True)

    start_time = time.time()

    train(config['max_epochs'], model, optimizer, scheduler, train_loader, valid_loader, f"{config['project_name']}_{model_name}")
...
def main():
    ...
    valid_loader = DataLoader(valid_dataset, batch_size=config['valid_batch_size'], collate_fn=collate)
    test_loader = DataLoader(test_dataset, batch_size=config['test_batch_size'], collate_fn=collate)
    ...
    if len(results) > 1:
        ...
        pd.DataFrame(results).to_csv(f"runs/run-{config['project_name']}/benchmark_results.csv", index=False)
